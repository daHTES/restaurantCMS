...
class MenuController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($request->image){
            $request->validate([
                'image' => 'nullable|file|image|mimes:jpeg,png,jpg|max:50000000'
            ]);
    ...
    public function update(Request $request, $id)
    {
        ...
        if($request->image){
            $request->validate([
                'image' => 'nullable|file|image|mimes:jpeg,png,jpg|max:50000000000'
            ]);
            ...
            $imageName = date('mdYHs').uniqid().'.'.$request->image->extension();
            $request->image->image->move(public_path('menu_images'), $imageName);
        }else{
            $imageName = $menu->image;
        }
